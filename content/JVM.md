...
### 6. 所谓的 JVM 内存模型指什么？即让你回答JVM 的 happens-before 原则是什么？

内存模型描述的是程序中各变量（线程共享变量）的访问规则，以及在实际计算机系统中将变量存储到内存和从内存读取出变量这样的低层细节。
...
7. 线程中断规则:interrupt()方法的调用先行发生于被中断线程的代码检测到中断事件的发生
8. 对象终结规则:对象的构造函数执行和结束先于finalize()方法
...
类加载过程主要分为五个阶段。加载—验证—准备—解析—初始化
1. 首先是加载阶段，它是将字节码从不同的数据源读取到JVM中，然后转化位JVM认可的数据结构（Class对象）。在加载阶段主要完成以下几件事：
* 通过一个类的名字获取定义此类的二进制字节流
* 将这个字节流锁表示的静态存储结构转化为方法区运行时数据结构
* 在内存中生成一个代表这个类的class对象，作为方法区的各个数据访问的入口

2. 然后是验证阶段，这是虚拟机安全的重要保障，JVM需要校验字节信息是否符合Java虚拟机规范，否则就被认为是Error，这样可以防止一些恶意信息。验证阶段主要可以分为四个动作：（1）文件格式校验（2）元数据校验（3）字节码校验（4）符号引用校验。
3. 之后是准备阶段，创建类或借口中静态变量，并初始化静态变量的初始值，这里的初始化只是侧重于分配空间，并不是赋值，例如说static int a = 1,准备阶段之后 a = 0,而不是1，要在初始化之后才会变成1。
4. 之后是解析阶段，是将常量池中的符号引用替换成直接引用。
5. 最后就是初始化阶段，这一步就是真正的去执行初始化的代码逻辑，包括静态字段的赋值动作，以及静态代码块的执行，父类的代码块优先于子类。
